...
# Create veth pairs
ip link add listen_pub_veth netns internet_ns type veth peer listener_veth netns listener_ns
ip link add dialer_pub_veth netns internet_ns type veth peer dialer_veth netns dialer_ns
ip link add relay_pub_veth netns internet_ns type veth peer relay_veth netns relay_ns
...
ip netns exec internet_ns ip addr add 10.0.0.1/24 dev relay_pub_veth
ip netns exec relay_ns ip addr add 192.168.254.1/24 dev relay_veth

ip netns exec internet_ns ip addr add 10.0.0.2/24 dev listen_pub_veth
ip netns exec listener_ns ip addr add 192.168.1.1/24 dev listener_veth

ip netns exec internet_ns ip addr add 10.0.0.3/24 dev dialer_pub_veth
