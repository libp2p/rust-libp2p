...
pub struct Upgrader {
    inner: SendWrapper<UpgraderInner>,
}

impl Upgrader {
    pub fn new() -> Self {
        Self {
            inner: SendWrapper::new(UpgraderInner::new()),
        }
    }
    ...
    pub(crate) async fn outbound(
        ...
        let fut = SendWrapper::new(self.inner.outbound(sock_addr, remote_fingerprint, id_keys));
        fut.await
    }
}
