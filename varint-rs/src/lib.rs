...
impl DecoderState {
	...
	fn decode_one(mut self, byte: u8) -> Result<BigUint, Self> {
		...
		if byte & 0x80 == 0 { Ok(self.accumulator) } else { Err(self) }
	}
}
...
pub fn decode<R: Read>(stream: R) -> io::Result<BigUint> {
	...
	if finished_cleanly { Ok(out) } else { Err(io::Error::from(io::ErrorKind::UnexpectedEof)) }
}
...
mod tests {
	...
	fn can_decode_basic_uint() {
		assert_eq!(BigUint::from(300u16), decode(&[0b10101100, 0b00000010][..]).unwrap());
	}

	#[test]
	fn can_decode_basic_uint_async() {
		let mut decoder =
			Decoder::new(iter_ok::<_, ::std::io::Error>(vec![0b10101100, 0b00000010]));

		assert_eq!(decoder.poll().unwrap(), Async::NotReady);
		assert_eq!(decoder.poll().unwrap(), Async::Ready(Some(BigUint::from(300u16))));
		assert_eq!(decoder.poll().unwrap(), Async::Ready(None));
	}
}
