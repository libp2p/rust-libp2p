...
impl SymmetricState {
    ...
    pub(crate) fn decrypt_and_hash(&mut self, ciphertext: &[u8]) -> Result<Vec<u8>, NoiseError> {
        let plaintext = self
            .cs
            .decrypt_with_ad(&self.h.as_bytes()[..], &ciphertext)?;
        self.mix_hash(ciphertext);
        Ok(plaintext)
    }
...
impl HandshakeState {
    ...
    pub(crate) fn write_message_b(&mut self, input: &[u8]) -> Result<Vec<u8>, NoiseError> {
        let mut output: Vec<u8> = Vec::new();
        if self.e.is_empty() {
            self.e = Keypair::new();
        }
        ...
        let mut ns = self
            .ss
            .encrypt_and_hash(&self.s.get_public_key().as_bytes()[..])?;
    ...
    pub(crate) fn write_message_c(
        &mut self,
        input: &[u8],
    ) -> Result<(Hash, Vec<u8>, CipherState, CipherState), NoiseError> {
        let mut output: Vec<u8> = Vec::new();
        let mut ns = self
            .ss
            .encrypt_and_hash(&self.s.get_public_key().as_bytes()[..])?;
