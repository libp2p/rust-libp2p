...
pub mod constants;
/// Implements a `ProtocolHandler` for the gossipsub protocol. 2nd highest level.
pub mod handler;
/// Configures gossipsub and coding and decoding from `rpc_proto.rs.` 2nd lowest level, along with topic.
pub mod protocol;
...
// Rules for transport over-the-wire via protobuf. Lowest level.
mod rpc_proto;
