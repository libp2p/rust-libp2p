...
- Optimize memory usage through use of Bytes in various places.
  `publish` now requires `data: impl Into<Bytes>` as opposed to `data: impl Into<Vec<u8>>`.
  `outbound_transform` now takes `data` as `Bytes`.
