...
pub struct Behaviour {
    ...
    verified_shreds: HashMap<(PeerId, MessageId), HashMap<ShredIndex, Shred>>,
...
impl Behaviour {
    ...
    pub fn broadcast(
        ...
        for shred in shreds.iter() {
            ...
            self.broadcast_shred_to_children_in_tree(shred.clone(), &tree);
        }
    ...
    pub fn validate_shred(
        ...
        let Some(expected_parent) = tree.get_parent() else {
            unreachable!("Local node is the leader, this is not possible at this point")
        };

        if expected_parent != sender {
            return Err(ShredValidationError::ParentVerificationFailed {
    ...
    fn handle_received_shred(&mut self, sender: PeerId, shred: Shred) {
        ...
        let message_shreds = self
            .verified_shreds
            .entry((shred.id.publisher, shred.id.message_id))
            .or_default();
