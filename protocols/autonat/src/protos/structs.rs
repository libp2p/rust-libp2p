...
pub struct Message<'a> {
    pub type_pb: Option<mod_Message::MessageType>,
    pub dial: Option<mod_Message::Dial<'a>>,
    pub dialResponse: Option<mod_Message::DialResponse<'a>>,
}
    ...
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                ...
                Ok(18) => msg.dial = Some(r.read_message::<mod_Message::Dial>(bytes)?),
                Ok(26) => msg.dialResponse = Some(r.read_message::<mod_Message::DialResponse>(bytes)?),
...
#[derive(Debug, Default, PartialEq, Clone)]
pub struct Dial<'a> {
    pub peer: Option<mod_Message::PeerInfo<'a>>,
}
...
impl<'a> MessageRead<'a> for Dial<'a> {
    fn from_reader(r: &mut BytesReader, bytes: &'a [u8]) -> Result<Self> {
        ...
        while !r.is_eof() {
            match r.next_tag(bytes) {
                Ok(10) => msg.peer = Some(r.read_message::<mod_Message::PeerInfo>(bytes)?),
                Ok(t) => { r.read_unknown(bytes, t)?; }
                Err(e) => return Err(e),
            }
        }
...
pub struct DialResponse<'a> {
    pub status: Option<mod_Message::ResponseStatus>,
