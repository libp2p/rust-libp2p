                        ...
                        match relay_circuit_addr_str.parse::<Multiaddr>() {
                            Ok(relay_circuit_addr) => {
                                ...
                                if let Err(e) = swarm.dial(relay_circuit_addr.clone()) {
                                    tracing::error!("Failed to dial relay circuit: {:?}", e);
                                    let _ = event_sender.unbounded_send(Event::Error { msg: format!("Dial failed to {}", relay_circuit_addr) });
                                    continue;
                                }
                                ...
                                tracing::info!("Also dialing WebRTC address: {}", webrtc_addr_str);

                                match webrtc_addr_str.parse::<Multiaddr>() {
                                    Ok(webrtc_addr) => {
                                        // This will create a Future that waits for the connection from behaviour
                                        if let Err(e) = swarm.dial(webrtc_addr) {
                                            tracing::error!("Failed to dial WebRTC: {:?}", e);
                                        }
                                    }
                                    Err(e) => {
                                        tracing::error!("Invalid WebRTC multiaddr: {:?}", e);
                                    }
                                }
                            }
                            Err(e) => {
                                tracing::error!("Invalid relay circuit multiaddr: {:?}", e);
                            }
                        }
                    }
                }
